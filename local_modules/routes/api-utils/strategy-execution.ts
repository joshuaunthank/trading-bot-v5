 ...
 */
export async function startStrategy(req: Request, res: Response): Promise<void> {
	...
	try {
		...
		console.log(`[Strategy API] Starting strategy: ${strategyConfig.name} with real indicators`);
		
		// Start strategy using the real StrategyManager with actual indicator calculations
		const startedStrategyId = await strategyManager.startStrategy(strategyConfig);
		...
		console.error(`[Strategy API] Failed to start strategy ${strategyId}:`, error);
		res.status(500).json({
			success: false,
...
export async function stopStrategy(req: Request, res: Response): Promise<void> {
	...
	try {
		...
		console.error(`[Strategy API] Failed to stop strategy ${strategyId}:`, error);
		res.status(500).json({
			success: false,
 ...
 */
export async function pauseStrategy(req: Request, res: Response): Promise<void> {
	const strategyId = req.params.id;
	...
	try {
		...
		console.error(`[Strategy API] Failed to pause strategy ${strategyId}:`, error);
		res.status(500).json({
			success: false,
 ...
 */
export async function resumeStrategy(req: Request, res: Response): Promise<void> {
	const strategyId = req.params.id;
	...
	try {
		...
		console.error(`[Strategy API] Failed to resume strategy ${strategyId}:`, error);
		res.status(500).json({
			success: false,
...
export function getAllStrategyStatus(req: Request, res: Response): void {
	try {
		...
		res.status(500).json({
			success: false,
			error: error instanceof Error ? error.message : "Failed to get strategy statuses"
		});
	}
}
...
export function getStrategyStatus(req: Request, res: Response): void {
	...
	try {
		...
		console.error(`[Strategy API] Failed to get strategy status ${strategyId}:`, error);
		res.status(500).json({
			success: false,
