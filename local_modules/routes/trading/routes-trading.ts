...
router.get("/init", async (req: Request, res: Response) => {
	try {
		...
		if (result) {
			res.status(200).json({
				success: true,
				message: "Trading service initialized successfully",
			});
		} else {
			res.status(500).json({
				success: false,
 ...
 */
router.delete("/order/:id/:symbol", async (req: Request, res: Response) => {
	try {
		if (!tradingService.isInitialized()) {
			await tradingService.init();
		}
		...
		if (result) {
			res
				.status(200)
				.json({ success: true, message: `Order ${id} cancelled successfully` });
		} else {
			res
				.status(500)
...
router.post("/close-position", async (req: Request, res: Response) => {
	try {
		...
		if (!symbol || !positionSide) {
			res.status(400).json({
				success: false,
				message: "Missing required parameters. Required: symbol, positionSide",
			});
			return;
		}
